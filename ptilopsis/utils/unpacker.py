...
class Unpacker:
    ...
    def __init__(
        ...
        self.http = http or make_client(headers=self.ua)
        """海外服官方 CDN 用的客户端。"""
    ...
    async def check_update(self, region="CN"):
        local_version = self.version[region]["resVersion"]
        if local_version != await self.get_version(region):
    ...
    async def check_all_update(self):
        flag = False
        for r in self.config:
            if r != "CN":
                logger.info(f"Start to check {r} server.")
                flag ^= await self.check_update(region=r)
        return flag

    async def get_version(self, region="CN"):
        ...
        """
        if region == "CN":
            return await self._get_version_torappu(region)
        return await self._get_version_official(region)

    async def _get_version_torappu(self, region: str) -> str:
        """国服:torappu 上 gamedata 已就绪的最新版本(funcVer 不再维护)。"""
        latest = await self.torappu.latest_version()
    ...
    async def _get_version_official(self, region: str) -> str:
        ...
        resp = await self.http.get(url)
        resp.raise_for_status()
        ret2 = json.loads(resp.json()["content"])
