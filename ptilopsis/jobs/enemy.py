...
def run(ctx: JobContext) -> None:
    ...
    enemy_race_dict = {
        r["id"]: r["raceName"] for r in enemy_handbook_table["raceData"].values()
    }
    enemy_sptype_dict = {
    ...
    for enemy in enemy_handbook_table["enemyData"].values():
        ...
        if enemy["enemyId"] in enemy_db_index:
            ...
            for idx, d in enumerate(enemy_data["Value"]):
                ...
                for idx_a, k in enumerate(
                    ...
                    if (
                        lv_data["attributes"][k]["m_defined"] is True
                        and attribute_data[idx_a] == -1
                    ):
                ...
                if (
                    lv_data["enemyTags"]["m_defined"] is True
                    and lv_data["enemyTags"]["m_value"]
                ):
                ...
                if lv_data["levelType"]["m_defined"] is True:
                    content_lv += f"\n|地位={enemy_level_dict.get(lv_data['levelType']['m_value'], '普通')}"
                elif idx == 0:
                    content_lv += "\n|地位=普通"
                if (
                    lv_data["enemyTags"]["m_defined"] is True
                    and lv_data["enemyTags"]["m_value"] is not None
                ):
                    content_lv += "\n|种类=" + ",".join(
                        enemy_race_dict.get(r, "未知")
                        for r in lv_data["enemyTags"]["m_value"]
                    )
                ...
                content_lv += get_value(
                    idx, lv_data["rangeRadius"], "攻击范围半径", "f"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["maxHp"], "最大生命值", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["atk"], "攻击力", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["def"], "防御力", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["magicResistance"], "法术抗性", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["moveSpeed"], "移动速度", "f"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["attackSpeed"], "攻击速度", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["baseAttackTime"], "攻击间隔", "f"
                )
                content_lv += get_value(
                    idx,
                    lv_data["attributes"]["hpRecoveryPerSec"],
                    "生命恢复速度",
                    "i",
                )
                content_lv += get_value(
                    idx,
                    lv_data["attributes"]["spRecoveryPerSec"],
                    "sp恢复速度",
                    "i",
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["massLevel"], "重量等级", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["epResistance"], "损伤抵抗", "i"
                )
                content_lv += get_value(
                    idx,
                    lv_data["attributes"]["epDamageResistance"],
                    "元素抗性",
                    "i",
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["tauntLevel"], "基础嘲讽等级", "i"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["stunImmune"], "眩晕抗性", "b"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["silenceImmune"], "沉默抗性", "b"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["sleepImmune"], "沉睡抗性", "b"
                )
                content_lv += get_value(
                    idx, lv_data["attributes"]["frozenImmune"], "冻结抗性", "b"
                )
                content_lv += get_value(
                ...
                content_lv += get_value(
                    idx, lv_data["attributes"]["palsyImmune"], "麻痹抗性", "b"
                )
                content_lv += get_value(
                ...
                if lv_data["spData"] is not None:
                    try:
                        ...
                        continue
                if lv_data.get("talentBlackboard"):
                    content_lv += "\n|天赋=<!--{}-->".format(
                        json.dumps(
                            lv_data["talentBlackboard"],
                            indent=4,
                            ensure_ascii=False,
                        )
                    )
        ...
        if race_tag.__len__() > 0:
            content += "\n|种类=" + ",".join(
                enemy_race_dict.get(r, "未知") for r in race_tag
            )
        ...
        if "linkEnemies" in enemy and enemy["linkEnemies"] != []:
            ...
            for link_e in enemy["linkEnemies"]:
                if link_e in enemy_handbook_table["enemyData"]:
                    link_e_list.append(
                        f"[[{enemy_handbook_table['enemyData'][link_e]['name'].strip()}]]"
                    )
                else:
                    for ee in enemy_database["enemies"]:
                        if ee["Key"] == link_e:
                            link_e_list.append(
                                f"[[{ee['Value'][0]['enemyData']['name']['m_value'].strip()}]]"
                            )
                            break
        ...
        spine_content["prefix"] = (
            f"https://torappu.prts.wiki/assets/enemy_spine/{enemy['enemyId']}/"
        )
...
def update_data(ctx: JobContext) -> None:
    ...
    for enemy in enemy_handbook_table["enemyData"].values():
        ...
        new_data = {
            ...
            "damageType": " ".join(
                enemy_damage_dict.get(x, "未知") for x in enemy["damageType"]
            ),
        ...
        new_data["ability"] = format_abilityList(
            enemy["abilityList"], rts_html, html=True
        )
...
@job
def update_summary(ctx: JobContext) -> None:
    enemy_handbook_table = ctx.getgd("excel/enemy_handbook_table.json")
    enemy_database = ctx.getgd("levels/enemydata/enemy_database.json")
    pass
...
@job
def update_immune(ctx: JobContext) -> None:
    def get_value(idx, v, name, k):
        if idx == 0 or v["m_defined"] == True:
            if v["m_defined"] == False:
                s = {"i": "0", "f": "0.0", "s": "", "b": "无"}.get(k, "")
    ...
    for enemy in enemy_handbook_table["enemyData"].values():
        if enemy["name"] == "-":
            continue
        old_page = ctx.wiki.read(enemy_list_2[enemy["name"].strip()].strip())
        new_page = old_page
