"""skill_table.json 的数据模型。

由 scripts/gen_gamedata_models.py 从 FBS/skill_table.fbs 生成,请勿手改;
字段语义见 dump.cs 里的同名类。
"""
...
class SkillDurationType(IntEnum):
...
class SkillDataBundleLevelData(GameDataModel):
    ...
    sp_data: SpData | None = None
    prefab_id: str | None = None
    duration: float = 0.0
    blackboard: list[BlackboardDataPair] | None = None


class SkillDataBundle(GameDataModel):
    """clz_Torappu_SkillDataBundle"""

    skill_id: str | None = None
    icon_id: str | None = None
    hidden: bool = False
    levels: list[SkillDataBundleLevelData] | None = None


# root_type clz_Torappu_SimpleKVTable_clz_Torappu_SkillDataBundle
# JSON 里去掉了外层的 skills,直接是 {id: ...}
SkillTable = TypeAdapter(dict[str, SkillDataBundle])


ItemBundle.model_rebuild()
SpData.model_rebuild()
BlackboardDataPair.model_rebuild()
SkillDataBundleLevelData.model_rebuild()
SkillDataBundle.model_rebuild()
