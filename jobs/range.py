...
svgHeadTemplate = '<svg xmlns="http://www.w3.org/2000/svg" ' \
...
def skill_range(wiki, raneg_table):
    for rangeId in raneg_table:
        ...
        svgStr = svgHeadTemplate.format(0, 0, 26 * rangeCol, 26 * rangeRow, 26 * rangeCol, 26 * rangeRow) + svgDefStr
        ...
        Str = '<includeonly>' + \
              ...
              rangeId + \
              '|width={}px|height={}px'.format(26 * rangeCol, 26 * rangeRow) + \
              '}}</noinclude>'

        wiki.edit(
            title = 'Widget:Range/' + rangeId,
            text = Str,
            summary = 'init',
            createonly = '1'
        )
