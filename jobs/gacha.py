...
def get_gacha_mainpage(character_table, gacha_data, rts):
    content = '===出现概率上升===\n'
    content += '<br/>'.join(['{rarity_star}\t占{rarity}★出率的{percent:.0%}<br/>{charIdList}'.format(
        rarity = op['rarityRank'] + 1,
        rarity_star = (op['rarityRank'] + 1) * '★',
        percent = op['percent'] * op['count'],
        charIdList = '/'.join([character_table[char_key]['name'] for char_key in op['charIdList']])
    ) for op in gacha_data['up']])

    content += '\n===全部可能出现的干员===\n'
    content += '<br/>'.join(['{rarity}\t出率{totalPercent:.0%}<br/>{charIdList}'.format(
        rarity = (op['rarityRank'] + 1) * '★',
...
def update_gacha_list(wiki):
    with open('test_gacha.txt', 'r', encoding = 'utf-8') as f:
...
class Gacha(Job):
    def _run(self):
        ...
        gacha_data = session.get('https://weedy.baka.icu/gacha/LIMITED_9_0_3').json()['detail']

        print('request success.')
        content = get_gacha_mainpage(character_table, gacha_data, rts)

        self.wiki.edit(
            title = '用户:Seniorious/test',
            text = content,
            summary = 'update'
        )
        # print(content)
