...
!解锁等级!!产品!!消耗材料!!消耗龙门币!!消耗心情!!副产品产出概率!!额外解锁条件'''
    ...
    for formula_id in formulas:
        ...
        require_stage = ', '.join(
            ['{}星通关[[{}]]'.format(stage['rank'], stage_table['stages'][stage['stageId']]['code']) for stage in
                formula['requireStages']]
        )
        if require_stage == '':
            require_stage = '－'
    ...
    formulas_text += '基建材料=\n' + table_title + ''.join(
        [formulas_content['F_BUILDING'][formula] for formula in formulas_content['F_BUILDING']]
    ) + '\n|}'
    formulas_text += '\n|-|\n' + '精英材料=\n' + table_title + ''.join(
        [formulas_content['F_EVOLVE'][formula] for formula in formulas_content['F_EVOLVE']]
    ) + '\n|}'
    formulas_text += '\n|-|\n' + '技巧概要=\n' + table_title + ''.join(
        [formulas_content['F_SKILL'][formula] for formula in formulas_content['F_SKILL']]
    ) + '\n|}'
    formulas_text += '\n|-|\n' + '芯片=\n' + table_title + ''.join(
        [formulas_content['F_ASC'][formula] for formula in formulas_content['F_ASC']]
    ) + '\n|}'
    formulas_text += '\n</tabber>'
...
class Formula(Job):
    def _run(self):
        ...
        self.wiki.edit(
            title = '用户:Seniorious/workshopFormulas',
            text = content,
            summary = 'update'
        )
        # print(content)
