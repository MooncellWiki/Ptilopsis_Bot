...
def concat_id(voice_data, char_name, flag_CN, text_jp=''):
    text = '|标题{id}={title}\n|日文{id}={text_jp}\n|中文{id}={text_cn}\n|语音{id}={voice}\n'.format(
...
def get_charword_data(word_key, file_name, charword_table, flag_CN, text_jp_dict=None, title='语音记录'):
    ...
    for data in sorted(filter(lambda x: x['wordKey'] == word_key, charword_table.values()),
        ...
        if text_jp_dict is not None and str(data['voiceIndex']) in text_jp_dict:
            char_word += '\n' + concat_id(data, file_name, flag_CN, text_jp = text_jp_dict[str(data['voiceIndex'])])
        else:
...
def create_charword(wiki, char_list, charword_table, voice_lang_dict):
    for char_id, char_name in char_list:
        ...
        for k in key_list:
            ...
            content += get_charword_data(k, file_name, charword_table, flag_CN) + '\n'
    ...
    for char_id, char_name in char_list:
        ...
        for k in key_list:
            ...
            if result:
                ...
                new_text += get_charword_data(k, file_name, charword_table, flag_CN, text_jp_dict = d, title = title)
            else:
...
def update_charword_jp(wiki, char_list, charword_table, voice_lang_dict, charword_table_jp, mode='JP'):
    for char_id, char_name in char_list:
        key_list = word_key_list(char_id, charword_table)
        # 处理阿米娅升变
        if char_id == 'char_1001_amiya2':
            ...
            if result:
                ...
                for k0, v0 in result_jp:
                    if k0 not in text_jp_dict:
                        text_jp_dict[k0] = v0
                # 处理end
                new_text += get_charword_data(k, file_name, charword_table, flag_CN, text_jp_dict = text_jp_dict, title = title)
            else:
                text_jp_dict = {str(d['voiceIndex']): d['voiceText'] for d in
                    filter(lambda x: x['wordKey'] == k, charword_table_jp.values())}
                new_text += get_charword_data(k, file_name, charword_table, flag_CN, text_jp_dict = text_jp_dict)
            new_text += '\n'
        origin_text = origin_text[:-2]
        if mode == 'US':
    ...
    def update_jp(self):
        ...
        for char_id in character_table_jp:
            ...
            if char_id in en_list:
                ...
                char_list.append((char_id, character_table[char_id]['name']))
        char_list.append(('char_1001_amiya2', '阿米娅(近卫)'))
        update_charword_jp(self.wiki, char_list, charword_table, voice_lang_dict, charword_table_jp)
        update_charword_jp(self.wiki, char_list_en, charword_table, voice_lang_dict, charword_table_en, mode='US')
