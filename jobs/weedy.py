...
|寻访池关闭时间={time_end}
    ...
    content = template.format(
        ...
        time_begin=datetime.fromtimestamp(
            good_list[0]["goodStartTime"], pytz.timezone("Asia/Shanghai")
        ).strftime("%Y-%m-%d %H:%M"),
        time_end=datetime.fromtimestamp(
            good_list[0]["goodEndTime"], pytz.timezone("Asia/Shanghai")
        ).strftime("%Y-%m-%d %H:%M"),
    )
    ...
    for stage in stage_list:
        ...
        for rune_key in stage["runes"]:
            rune_list[stage["runes"][rune_key]["points"]].append(
                "|{addition}{{{{危机合约词条|{key}|{name}|{point}|{desc}}}}}".format(
                    addition='style="position:relative;"|{{合约标记|挑战}}'
                    if rune_key in mission_list
                    else "",
                    key=rune_key,
        ...
        if rune_text[1] != "":
            text += (
                ...
                + "\n"
                + rune_text[1]
                .replace('"|{{合约标记', '" width=50px|{{合约标记')
                .replace("|{{危机合约词条", "|width=50px|{{危机合约词条")
                .replace("|\n", "|width=50px|\n")
            )
        if rune_text[2] != "":
            text += rank2 + "\n" + rune_text[2]
