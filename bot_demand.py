...
def update_mat_demand(se, url, character_table, item_table):
        ...
        if char_detail['profession'] == 'TRAP' or char_detail['profession'] == 'TOKEN':
            ...
            for phase_id in range(1, len(char_detail['phases'])):
                for material_id in range(len(char_detail['phases'][phase_id]['evolveCost'])):
                    mat_add(1, char_detail['phases'][phase_id]['evolveCost'][material_id]['id'], char, char_detail['phases'][phase_id]['evolveCost'][material_id]['count'])
            ...
            if char_detail['skills']:
                for allSkillLvlup_id in range(len(char_detail['allSkillLvlup'])):
                    for common_material_id in range(len(char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'])):
                        mat_add(2, char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'][common_material_id]['id'], char, char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'][common_material_id]['count'])
    ...
    for material in mat_dic:
        ...
        num_flag1 = fin2.find('==干员需求==')
        num_flag2 = fin2.find('==材料掉落==')
        if num_flag2 != -1:
            fin = fin2[:num_flag1] + mat_desc + fin2[num_flag2:]
        else:
            fin = fin2[:num_flag1] + mat_desc + '<references/>\n{{道具导航}}'

        if fin != fin2:
            num_trial = 5
            for i in range(num_trial):
                try:
                    write_wiki(se, url, item_table['items'][material]['name'], fin, '')
                    # print(fin)
                    print('Update: {}.'.format(item_table['items'][material]['name']))
                    break
                except:
                    if i < num_trial:
                        print('Write {} fail. Try again.'.format(item_table['items'][material]['name']))
                    else:
                        print('Write {} fail. Skip.'.format(item_table['items'][material]['name']))
        else:
