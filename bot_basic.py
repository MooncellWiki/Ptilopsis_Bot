...
def get_phases_data(char_detail):
    ...
    for phases_num in range(len(char_detail['phases'])):
        if phases_num == 0:
            ...
            #     print('error')
            #     break
            phases_data += '|精英0_1级_生命上限=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][0]['data']['maxHp']) + '\n' + '|精英0_1级_攻击=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][0]['data']['atk']) + '\n' + '|精英0_1级_防御=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][0]['data']['def']) + '\n' + '|精英0_1级_法术抗性=' + str(int(char_detail['phases'][phases_num]['attributesKeyFrames'][0]['data']['magicResistance'])) + '\n'
            phases_data += '|精英0_满级=' + str(char_detail['phases'][phases_num]['maxLevel']) + '\n'
            phases_data += '|精英0_满级_生命上限=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['maxHp']) + '\n' + '|精英0_满级_攻击=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['atk']) + '\n' + '|精英0_满级_防御=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['def']) + '\n' + '|精英0_满级_法术抗性=' + str(int(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['magicResistance'])) + '\n'
        else:
            # if len(char_detail['phases'][phases_num]['attributesKeyFrames']) != 2:
            #     print('error')
            #     break
            phases_data += '|精英' + str(phases_num) + '_满级=' + str(char_detail['phases'][phases_num]['maxLevel']) + '\n'
            phases_data += '|精英' + str(phases_num) + '_满级_生命上限=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['maxHp']) + '\n' + '|精英' + str(phases_num) + '_满级_攻击=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['atk']) + '\n' + '|精英' + str(phases_num) + '_满级_防御=' + str(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['def']) + '\n'  + '|精英' + str(phases_num) + '_满级_法术抗性=' + str(int(char_detail['phases'][phases_num]['attributesKeyFrames'][1]['data']['magicResistance'])) + '\n'

    favorKey_data = '|信赖加成_生命上限=' + str(char_detail['favorKeyFrames'][1]['data']['maxHp']) + '\n' + '|信赖加成_攻击=' + str(char_detail['favorKeyFrames'][1]['data']['atk']) + '\n' + '|信赖加成_防御=' + str(char_detail['favorKeyFrames'][1]['data']['def']) + '\n'
...
def get_talent_list(char_detail):
    ...
    for talent_id in range(len(char_detail['talents'])):
        ...
        for talent_table_id in range(len(talent_table)):
            ...
            talent_condition = get_tal_condition(talent_table[talent_table_id]['requiredPotentialRank'], talent_table[talent_table_id]['unlockCondition']['phase'], talent_table[talent_table_id]['unlockCondition']['level'])

            talent_list += '|第' + trans_id(talent_id+1) + '天赋' + str(talent_table_id+1) + '=' + talent_table[talent_table_id]['name'] + '\n'
            talent_list += '|第' + trans_id(talent_id+1) + '天赋' + str(talent_table_id+1) + '条件=' + talent_condition + '\n'
            talent_list += '|第' + trans_id(talent_id+1) + '天赋' + str(talent_table_id+1) + '效果=' + talent_description + '\n'
    talent_list += '}}'
    return talent_list
...
def get_potential_list(char_detail):
    if char_detail['potentialRanks']:
        ...
        for potential_id in range(len(char_detail['potentialRanks'])):
            potential_list += '|潜能' + str(potential_id+2) + '=' + char_detail['potentialRanks'][potential_id]['description'] + '\n'
        potential_list += '}}'
    else:
    ...
    if char_detail['skills']:
        for skill_id in range(len(char_detail['skills'])):
            ...
            skill_list += '\n\'\'\'技能{num}（{skill_cond}开放）\'\'\'\n{{{{技能\n|技能名={skill_name}\n|技能类型1={type1}{type2}'.format(
                num=skill_id+1,
                skill_cond=get_tal_condition(0,char_detail['skills'][skill_id]['unlockCond']['phase'],char_detail['skills'][skill_id]['unlockCond']['level']),
            ...
            for level_id in range(len(skill_table[char_detail['skills'][skill_id]['skillId']]['levels'])):
                ...
                skill_description = skill_data['levels'][level_id]['description'].replace('-{-','{').replace('{-','{').replace('\\n','<br/>')
                skill_description = replace_key(replace_upper(skill_description))
                skill_description = skill_description.replace(':0%}',':.0%}').replace(':0.0%}',':0.1%}').replace(':0.0}','}')
    ...
    if len(char_detail['phases']) >= 2:
        for phase_id in range(1, len(char_detail['phases'])):
            ...
            for material_id in range(len(char_detail['phases'][phase_id]['evolveCost'])):
                material_list += ' {{材料消耗|' + item_table['items'][char_detail['phases'][phase_id]['evolveCost'][material_id]['id']]['name'] + '|' + str(char_detail['phases'][phase_id]['evolveCost'][material_id]['count']) + '}}'
        ...
        for allSkillLvlup_id in range(len(char_detail['allSkillLvlup'])):
            ...
            for common_material_id in range(len(char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'])):
                if common_material_list == '':
                    common_material_list = '{{材料消耗|' + item_table['items'][char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'][common_material_id]['id']]['name'] + '|' + str(char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'][common_material_id]['count'])+ '}}'
                else:
                    common_material_list += ' {{材料消耗|' + item_table['items'][char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'][common_material_id]['id']]['name'] + '|' + str(char_detail['allSkillLvlup'][allSkillLvlup_id]['lvlUpCost'][common_material_id]['count'])+ '}}'
            ...
            if char_detail['skills'][skill_id]['levelUpCostCond']:
                for i in [8,9,10]:
                    ...
                    for material_id in range(len(skill_levelup_material)):
                        if material_list == '':
                            material_list = '{{材料消耗|' + item_table['items'][skill_levelup_material[material_id]['id']]['name'] + '|' + str(skill_levelup_material[material_id]['count']) + '}}'
                        else:
                            material_list += ' {{材料消耗|' + item_table['items'][skill_levelup_material[material_id]['id']]['name'] + '|' + str(skill_levelup_material[material_id]['count']) + '}}'
...
def get_stories_list(char_detail, stories_table, char_key):
    ...
    for stories_id in range(len(char_stories['storyTextAudio'])):
        ...
        if storyCondition_id == 0:
            ...
            storyCondition = replace_story_condition(char_stories['storyTextAudio'][stories_id]['stories'][0]['unLockString'], char_stories['storyTextAudio'][stories_id]['stories'][0]['unLockParam'])
        stories_list += '|档案' + str(stories_id+1) + '=' + storyTitle + '\n|档案' + str(stories_id+1) + '条件=' + storyCondition + '\n|档案' + str(stories_id+1) + '文本=' + storyText + '\n'
    stories_list += '}}'
...
def replace_key(text):
    ...
    result = re.search(pattern, text)
    if result:
        text = replace_key(result.group(1)) + '{' + result.group(2).replace('.', '').replace(']', '').replace('[', '') + replace_key(result.group(3))
    return text
