...
- ``[dict__K__V]`` → ``dict[K, V]``;``[list_T]`` → ``list[list[T]]``;
  ``enum__X`` 字段 → ``str``,同时生成 ``class X(IntEnum)`` 供比较顺序。
...
class Generator:
    def __init__(self, schema: Schema, module_name: str) -> None:
        self.schema = schema
        self.module_name = module_name
        # dict__ / list_ / kvp__ 这些容器表不生成类,只当类型构造器用
    ...
    def is_container_table(self, name: str) -> bool:
        ...
        return name.startswith(("dict__", "list_", "kvp__", SIMPLE_KV_PREFIX))
    ...
    def render(self) -> str:
        ...
        header = (
            f'"""{self.module_name}.json 的数据模型。\n\n'
            f"由 scripts/gen_gamedata_models.py 从 FBS/{self.module_name}.fbs 生成,"
            "请勿手改;\n字段语义见 dump.cs 里的同名类。\n"
            '"""\n\n'
        )
...
def generate(table_name: str) -> Path:
    ...
    code = Generator(schema, table_name).render()
    out = OUT_DIR / f"{table_name}.py"
